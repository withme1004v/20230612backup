...
EC$(function() {
    ...
    EC$('#popup_close_btn').click(function() {
        if (EC$('#popup_close_check').prop('checked') == true) {

            var oCheckedData = splitKeyValue(EC$('#popup_close_check').prop('value'));
            var aPopupCookie = cookieToArray(EC$.cookie('SDE_POPUP'));
            ...
            EC$.cookie('SDE_POPUP', aSaveCookie.join('&'), { expires: 30, path: '/'});
        }

        if (POPUP_TYPE == 'L') {
            var oNode = EC$(window.parent.document.body).find('#popup_'+POPUP_IDX);
    ...
    if (chkPrv != null) {//ECHOSTING-85488
        ...
        // 윈도우 팝업
        if (opener != undefined && opener.EC$('#frmNewPopup [name="foot_design_type"]:checked').val() != undefined) {
            //var sFooterId = '#foot_design_preview_' + ((opener.EC$('#frmNewPopup [name="foot_design_type"]:checked').val() == 'B') ? 'base' : 'detail');
            EC$('.xans-popup-footer').html(opener.EC$(sFooterId)[0].outerHTML);
        }
        // 레이어 팝업
        if (parent.parent.EC$('#frmNewPopup [name="foot_design_type"]:checked').val() != undefined) {
            //var sFooterId = '#foot_design_preview_' + ((parent.parent.EC$('#frmNewPopup [name="foot_design_type"]:checked').val() == 'B') ? 'base' : 'detail');
            EC$('.xans-popup-footer').html(parent.parent.EC$(sFooterId)[0].outerHTML);
        }
    }
    ...
    getData: function()
    {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (aStorageData != null && aStorageData.length > 1) {
                ...
                EC$.ajax({
                    url: '/exec/front/manage/async?module=BasketProduct',
                    success: function (aData) {
