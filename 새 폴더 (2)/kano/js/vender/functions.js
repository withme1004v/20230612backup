...
function splittingTextDelay (object, speed, delay_speed) {
	var splitLength = $(object).find("span").length;
	for (var i=0; i<splitLength; i++) {
		if (  $(object).data("css-property") == "animation" ) {
			$(object).find("span").eq(i).css("animation-delay",delay_speed+(i*speed)+"s");
		}else if( $(object).data("css-property") == "transition" ) {
			$(object).find("span").eq(i).css("transition-delay",delay_speed+(i*speed)+"s");
		}
	}
}
